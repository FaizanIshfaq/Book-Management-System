...
class BookTest {
	...
	void testBook() {
		...
		assertEquals(expectedLines3, this.siddharthaBook.getLines().subList(0, 3), "The expected 3 lines do not match the actual first 3 'Siddhartha' book lines.");
		...
		assertEquals(expectedLines2, this.warAndPeaceBook.getLines().subList(0, 3), "The expected 3 lines do not match the actual first 3 'Siddhartha' book lines.");
	...
	void testGetTitle() {
		...
		assertEquals("War and Peace", this.warAndPeaceBook.getTitle(), "The expected title does not match the actual 'War and Peace' book title.");
		assertEquals("Siddhartha", this.siddharthaBook.getTitle(), "The expected title does not match the actual 'Siddhartha' book title.");
	...
	void testGetAuthor() {
		...
		assertEquals("Leo Tolstoy", this.warAndPeaceBook.getAuthor(), "The expected author does not match the actual 'War and Peace' book author.");
		assertEquals("Hermann Hesse", this.siddharthaBook.getAuthor(), "The expected author does not match the actual 'Siddhartha' book author.");
	...
	void testGetTotalWordCount() {
		...
		//create list and book
		bookLines = new ArrayList<String>();
		bookLines.add("To be or not to be, that is the question.");
		book = new Book(bookLines);
		//confirm total word count
		assertEquals(10, book.getTotalWordCount(), "The total word count is incorrect. The expected answer is 10.");


		bookLines = new ArrayList<String>();
		bookLines.add("In the beginning God created the heavens and the earth.");
		bookLines.add("And the earth was without form, and void; and darkness was upon the face of the deep.");
		bookLines.add("And the Spirit of God moved upon the face of the waters.");
		book = new Book(bookLines);

		//confirm total word count
		assertEquals(39, book.getTotalWordCount(), "The total word count is incorrect. The expected answer is 29.");
	...
	void testGetUniqueWordCount() {
		...
		//confirm unique word count is 0
		assertEquals(0, book.getUniqueWordCount(), "The unique word count for an empty book should be 0.");
		assertTrue(this.warAndPeaceBook.getUniqueWordCount() >= 15000 && this.warAndPeaceBook.getUniqueWordCount() <= 20000, "The approximate unique word count for 'War and Peace' is incorrect. It should be greater than or equal to 15000 and less than or equal to 20000.");

		bookLines = new ArrayList<String>();
		bookLines.add("To be or not to be, that is the question.");
		book = new Book(bookLines);
		//confirm unique word count
		assertEquals(8, book.getUniqueWordCount(), "The unique word count is incorrect. The expected answer is 8.");
	...
	void testGetSpecificWordCount() {
		...
		book = new Book(bookLines);
		//confirm specific word count
		assertEquals(2, book.getSpecificWordCount("to"), "The specific word count is incorrect. The expected answer is 1.");
		assertTrue(this.warAndPeaceBook.getSpecificWordCount("love") >= 400 && this.warAndPeaceBook.getSpecificWordCount("love") <= 500, "The approximate specific word count of the word 'war' in 'War and Peace' is incorrect. It should be greater than or equal to 100 and less than or equal to 200.");
		assertTrue(this.siddharthaBook.getSpecificWordCount("hate") >= 1 && this.siddharthaBook.getSpecificWordCount("hate") <= 10, "The approximate specific word count of the word 'siddhartha' in 'Siddhartha' is incorrect. It should be greater than or equal to 100 and less than or equal to 200.");
